...
APP_NAME := "terraform-provider-addy"

# VERSION := `rg -No "\d+\.\d+\.\d+" internal/about/version.go`
...
build:
    go mod tidy
    go build -o "cmd/{{ APP_NAME }}" build/main.go
...
clean:
    ...
    @docker image rm $(docker image inspect "{{ APP_NAME }}-cli:{{ VERSION }}" | jq '.[].Id' | rg -o "sha256:(.{12})" | cut -d: -f2)
